...
<Note>
  Macroa Pulse is not yet published to PyPI. Install from source until a release is available.
</Note>

<Steps>
  <Step title="Clone and install from source">
    ```bash
    ...
    This installs the package in editable mode along with its core dependencies:
...
To use the CLI with an LLM provider for escalation responses, install the optional dependency:
